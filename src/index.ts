import {
  DERIVE_KEY_SALT_BYTE_LENGTH,
  generateCredentialHash,
  generateCredentialKey,
  generateRandomKey,
  getOrCreateDeriveKeySalt,
  matchOrCreateCredentialHash
} from "keyGen";
...
import {combineUint8Arrays, splitUint8ArrayAtOffsets} from "./arrayUtil";

/* Checks to see if a context was previously opened via open() or openFromRemoteKey(). This can be useful
...
/* Clears any information used to verify credentials or generate credential keys. Read the warning before calling this.
   
   WARNING: After this call, you won't be able to generate the same key from credentials. If you've got user data 
   encrypted with it, and no remote key solution, that data will be bricked.
 */
...
/* Creates a key that is suitable for storing on a web service for future retrieval. This key is entirely decoupled
   from user credentials. This has the advantage of avoiding the need for user data re-encryption when credentials 
   change. Disadvantages: Storing the credential key on a remote server means an attacker who gains access to it, could directly
   use the key to decrypt user data. The user won't be able to perform a local authentication to access their data 
   while offline.
   
   WARNING: Do not put this key in any form of persistent storage.
   
   @returns Base64-encoded string. */
export function createRemoteKey():string {
  const keyBytes = generateRandomKey(); // If you decide to implement server-side key generation instead of using this code, be certain to use a cryptographically-secure pseudo-random number generator.
  const deriveKeySalt = getOrCreateDeriveKeySalt(); // This value would be relevant if app code uses credential-derived keys later. The randomly-generated keyBytes are unaffected by it.
  const messageBytes = combineUint8Arrays([deriveKeySalt, keyBytes]);
  return bytesToBase64(messageBytes);
}

/* Creates a key that is suitable for storing on a web service for future retrieval. This key is derived from
   credentials. This has the advantage of allowing the user to perform a local authentication to access their data
   while offline, while still retaining an online backup to handle cases like a user forgetting their credentials.
   Disadvantages: Storing the credential key on a remote server means an attacker who gains access to it, could directly
   use the key to decrypt user data. And it will be necessary to re-encrypt user data when their credentials change.
   
   WARNING: Do not put this key in any form of persistent storage.
   
   @returns Base64-encoded string. */
export async function createRemoteKeyFromCredentials(userName:string, password:string):Promise<string> {
  const credentialHash = await generateCredentialHash(userName, password);
  if (!matchOrCreateCredentialHash(credentialHash)) throw Error('Credentials do not match previously used.');
  const credentialKeyBytes = await generateCredentialKey(userName, password);
  return bytesToBase64(credentialKeyBytes);
}

export function openFromRemoteKey(remoteKey:string):WearContext {
  const messageBytes = base64ToBytes(remoteKey);
  const [deriveKeySalt, keyBytes] = splitUint8ArrayAtOffsets(messageBytes, [DERIVE_KEY_SALT_BYTE_LENGTH]);
  setDeriveKeySalt(deriveKeySalt); // Setting this value locally from the remote key allows credential-derived keys to work for the same user credentials on different browsers/devices.
  return new WearContext(keyBytes, true);
}
...
/** A function that can be used to re-encrypt data. It performs 1. decrypt with key from old context and then 
    ...
    @return Promise to same data re-encrypted with the key from the new context.
 */
...
/* Returns a function that can be used by caller to re-encrypt data so it's accessible via a new context. This is
   ...
   @return Function in form of `async function reEncrypt(oldEncryptedData:Uint8Array):Promise<Uint8Array>`.
 */
...
/** Callback function to app-supplied re-encryption logic. In your implementation, you will want to 
    ...
    @return True if you completed all re-encryption without failure, false if not. In the latter, roll back any changes
            to avoid bricking user data.
 */
...
/** Returns a context derived from new credentials and calls a specified callback that will perform re-encryption.
    ...
    @return Promise to new context generated from new credentials if everything was successful.
 */
export async function changeCredentialsAndReEncrypt(oldContext:WearContext, newUserName:string, newPassword:string, onReEncrypt:IReEncryptCallback):Promise<WearContext> {
  if (oldContext.isClear()) throw Error('oldContext is unusable because it was closed.');
  const newContext = new WearContext(await generateCredentialKey(newUserName, newPassword), false);
...
export async function open(userName:string, password:string):Promise<WearContext | null> {
  ...
  const credentialKeyBytes = await generateCredentialKey(userName, password);
  return new WearContext(credentialKeyBytes, false);
}
...
export async function encrypt(context:WearContext, value:any):Promise<Uint8Array> {
  ...
  const credentialKey = await (context as any).dangerouslyGetCredentialKey();
